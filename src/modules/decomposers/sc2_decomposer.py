...
class SC2Decomposer:
    def __init__(self, args):
        ...
        if "change" in self.map_name:
            self.map_name, change_mode = self.map_name.split("change")
    ...
    def get_obs_size(self):
        ...
        nf_en = 4 + self.unit_type_bits # 4 + 0

        if self.obs_all_health: # True
            nf_al += 1 + self.shield_bits_ally
            nf_en += 1 + self.shield_bits_enemy # 1 + 1
