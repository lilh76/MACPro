...
from modules.mixers.qmix_macpro_attn import QMixerMACPro as QMixer
import torch as th
...
class QLearnerMACProAttn:
    def __init__(self, mac, logger, main_args):
        ...
        if main_args.mixer is not None:
            if main_args.mixer == "vdn":
                ...
                self.mixer = QMixer(self.surrogate_decomposer, main_args)
            else:
        ...
        self.global_params     = list(self.mac.state2feature_transformer.parameters()) + \
                                 list(self.mac.global_encoder.parameters()) + \
                                 self.mac.world_model_parameters()
        self.individual_params = list(self.mac.obs2feature.parameters()) + \
                                 list(self.mac.individual_encoder.parameters())
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, task, is_first_task):
        ...
        if self.mixer is not None:
            chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], self.task2decomposer[task])
            target_max_qvals = self.target_mixer(target_max_qvals, batch["state"][:, 1:], self.task2decomposer[task])
    ...
    def train_probing(self, probing_batch_dict, t_env):
        ...
        for task, batch_lst in probing_batch_dict.items():
            ...
            state_transformer = self.mac.state2feature_transformer(state, self.task2decomposer[task], self.task2n_agents[task])
            traj_z, mu_lgvar = self.mac.global_encoder(state_transformer[:,:-1,:], mask)
            ...
            if self.args.rec_coef > 0:
                ...
                state_for_model = self.mac.state2feature_model(state, self.task2decomposer[task], self.task2n_agents[task])
                action = th.cat([batch["actions_onehot"] for batch in batch_lst])[:,:-1,:,:]
                n_agents = action.shape[2]
                obs_model_inputs = obs[:,:-1,:].reshape(bs, seq_len, n_agents, -1)
                oa = th.cat((obs_model_inputs, action), dim = -1)
                oa = oa.permute(0,2,1,3).reshape(bs * n_agents, seq_len, -1)
                oa_embedding = self.mac.oa2feature(oa, task).reshape(bs, n_agents, seq_len, -1)
                oa_embedding = oa_embedding.mean(dim = 1)
                z4model = th.cat([traj_z.unsqueeze(1) for _ in range(seq_len)], dim = 1)
                model_inputs = th.cat((state_for_model[:,:-1,:], oa_embedding, z4model), dim = -1)
                model_rec = self.mac.world_model(model_inputs)
                state_embedding = model_rec[:, :, self.args.entity_embed_dim + 1:]
                state_rec = self.mac.task2rec_state[task](state_embedding)
                obs_embedding = model_rec[:, :, self.args.entity_embed_dim + 1: self.args.entity_embed_dim + 1 + self.args.entity_embed_dim]
                obs_embedding = th.cat([obs_embedding] * n_agents, dim = -1).reshape(bs, seq_len, n_agents, -1)
                id = th.eye(n_agents).unsqueeze(0).unsqueeze(0).repeat(bs, seq_len, 1, 1).to(self.args.device)
                obs_embedding = th.cat((obs_embedding, id), dim = -1)
                obs_rec = self.mac.task2rec_obs[task](obs_embedding).reshape(bs, seq_len, -1)
                pred_reward = model_rec[:, :, -1].unsqueeze(-1)
                model_rec = th.cat((state_rec, obs_rec, pred_reward), dim = -1)
            ...
            obs = self.mac.obs2feature(obs, task).reshape(bs * n_agents, seq_len, -1)           
        ...
        if self.args.contrastive_coef_individual > 0 and len(probing_batch_dict) >= 2:
            lst = []
            lst_ = list(individual_mu_lgvar_dict.values())
            min_n_agents = min(lst_, key=lambda x:x.shape[0]).shape[0] // bs
            for i, mu_lgvar in enumerate(lst_):
                task_id = (th.ones_like(mu_lgvar[:,0].unsqueeze(-1)) * i).to(self.args.device)
                mu_lgvar_ = th.cat((task_id, mu_lgvar), dim = -1)
                n_agents = mu_lgvar.shape[0] // bs
                if n_agents > min_n_agents:
                    random_idx = random.sample([i for i in range(mu_lgvar_.shape[0])], bs * min_n_agents)
                    mu_lgvar_ = mu_lgvar_[random_idx]
                lst.append(mu_lgvar_)
            mu_lgvar = th.cat(lst)
            random_idx = random.sample([i for i in range(mu_lgvar.shape[0])], bs)
