...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        if "change" in map_name:
            self.map_name = map_name.split("_change")[0]
            self.sight_range -= 1
            self.shoot_range -= 1
        else:
    ...
    def get_obs_agent(self, agent_id):
        ...
        if self.replace_obs:
            agent_obs = np.concatenate((agent_obs,self.get_state()),axis=0)
    ...
    def get_obs_size(self):
        ...
        if self.replace_obs:
            return move_feats + enemy_feats + ally_feats + own_feats + self.get_state_size()
